...
PARALLEL_JOBS=1 # Séquentiel pour la barre de progression
...
# CORRECTION BUFFERING POUR LA PROGRESSION
STDBUF_CMD=""
if command -v stdbuf &> /dev/null; then
    # -oL force le "Line Buffering" sur la sortie standard
...
convert_file() {
    ...
    if [[ -z "$duration_secs" ]]; then
        # Valeur par défaut si la durée n'est pas trouvée pour éviter division par zéro dans awk
        duration_secs=1 
    fi
    ...
    # if ! pv -f -p -t -e -s "$sizeBeforeBytes" "$file_original" > "$tmp_input"; then
    #     echo "$(date '+%Y-%m-%d %H:%M:%S') | ERROR PV copy failed | $file_original" >> "$LOG_ERROR"
    #     rm -f "$tmp_input" 2>/dev/null; return 1
    # fi
	
	# pv est utilisé ci-dessous sans options de progression (-f -p -t -e) pour éviter le conflit
    # avec la barre de progression de l'encodage NVENC/awk.
    ...
    # Utilisation de $STDBUF_CMD pour forcer l'affichage ligne par ligne
	# Au besoin tester avec -loglevel error
    if $IO_PRIORITY_CMD $STDBUF_CMD ffmpeg -y -loglevel warning \
    ...
    awk -v DURATION="$duration_secs" '
        ...
        /out_time_ms/ {
            ...
            printf "  → Encodage NVENC en cours... (%.1f%%)\r", p;
            fflush();
        }
    ' ; then 
        echo ""; echo "  ✅ Fichier converti."
        ...
        rm -f "$tmp_input" "$tmp_output" 2>/dev/null
    fi
}

export -f convert_file
export DRYRUN LOG_SUCCESS LOG_SKIPPED LOG_ERROR LOG_PROGRESS SUMMARY_FILE TMP_DIR NVENC_PRESET CRF IO_PRIORITY_CMD SOURCE OUTPUT_DIR REMOVE_ORIGINAL MAXRATE BUFSIZE BITRATE_CONVERSION_THRESHOLD_KBPS SKIP_TOLERANCE_PERCENT STDBUF_CMD
...
if [[ "$TEST_MODE" == true ]]; then
    ...
    # shuf mélange les lignes, head prend les N premières, puis on re-trie par taille (sort -nrk1,1)
