...
_handle_existing_queue() {
    # Gestion du fichier queue existant (demande à l'utilisateur si on doit le conserver)
    if [[ ! -f "$QUEUE" ]]; then
        return 1
    fi
...
_generate_new_queue() {
    # Génération de la queue complète (indexation + tri par taille décroissante)
    ...
    # Deuxième passe : indexer les fichiers
    local queue_tmp="$QUEUE.tmp"
    _index_video_files "$exclude_dir_name" "$total_files" "$queue_tmp" "$count_file"
    ...
    # Tri initial par taille décroissante et conversion en null-separated
    sort -nrk1,1 "$queue_tmp" | cut -f2- | tr '\n' '\0' > "$QUEUE"
    rm "$queue_tmp"
}
...
build_queue() {
    # Étape 1 : Tenter d'utiliser une queue personnalisée
    if _handle_custom_queue; then
        :
    # Étape 2 : Tenter de réutiliser la queue existante (avec demande à l'utilisateur)
    elif _handle_existing_queue; then
        # La queue existante a été acceptée ou a dû être régénérée
        if [[ ! -f "$QUEUE" ]]; then
            _generate_new_queue
        fi
    # Étape 3 : Générer une nouvelle queue
    else
        _generate_new_queue
    fi
    
    # Étape 4 : Appliquer les limitations (limit, random)
    _apply_queue_limitations
...
export_for_parallel() {
    export -f convert_file get_video_metadata should_skip_conversion clean_number colorize_pv_output \
        ...
        _handle_custom_queue _handle_existing_queue _count_total_video_files _index_video_files \
        _generate_new_queue _apply_queue_limitations _validate_queue_not_empty \
        _display_random_mode_selection _create_readable_queue_copy build_queue validate_queue_file
