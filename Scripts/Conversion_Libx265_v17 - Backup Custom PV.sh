...
custom_pv() {
    ...
    while kill -0 "$dd_pid" 2>/dev/null; do
        ...
        BEGIN{
            ...
            # Clear to end of line first to remove any leftover characters from previous longer lines
            # Format: elapsed [speed] [bar] PCT% SIZE  -> e.g. "0:00:03 [1.64GiB/s] [===>] 100% 4.34GiB"
            # Display transferred size at the end (copied) so user sees progress amount
            printf("\r\033[K%s%s%s", color, sprintf("%s [%5.2fGiB/s] [%s] %3d%% %s", hms(elapsed), (speed/(1024*1024*1024)), bar, pct, hr(copied)), nocolor);
    ...
    BEGIN{
        ...
        for(i=0;i<filled;i++) bar=bar"=";
        if(filled<width) bar=bar">"; for(i=filled+1;i<width;i++) bar=bar" ";
        # Final line: clear to end of line then print colored content and newline
        # Same ordering as above: elapsed [speed] [bar] PCT% SIZE\n
        # Print the transferred size (hr(copied)) at the end so user sees bytes moved
        printf("\r\033[K%s%s%s\n", color, sprintf("%s [%5.2fGiB/s] [%s] %3d%% %s", hms(elapsed), (speed/(1024*1024*1024)), bar, pct, hr(copied)), nocolor);
    }
    ' >&2
...
_execute_conversion() {
    ...
    local threads_per_job=$(($(nproc 2>/dev/null || echo "$PARALLEL_JOBS") / PARALLEL_JOBS))
