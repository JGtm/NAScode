...
# BUFSIZE : Taille du tampon VBV. Généralement 1.5x MAXRATE (Ex. : 4500 si MAXRATE=3000k).
# MAXRATE : Débit binaire maximal (en kilobits/seconde). Ex. : 3000 (1080p standard)
# VIDEO_BITRATE : Débit moyen souhaité pour la vidéo. Ex. : 3000 (1080p standard)
# Laisser vide pour permettre à ffmpeg de choisir la valeur par défaut.
BUFSIZE=""
MAXRATE=""
VIDEO_BITRATE=""
...
readonly BITRATE_CONVERSION_THRESHOLD_KBPS=2500
...
set_conversion_mode_parameters() {
    case "$CONVERSION_MODE" in
        film)
            ...
            ;;
        serie)
            # Valeurs recommandées pour les séries : bon compromis taille/qualité
            CRF=26
            RC_LOOKAHEAD=30
            ;;
        *)
...
cleanup() {
    touch "$STOP_FLAG"
    rm -f "$LOCKFILE"
    # Terminer les jobs en arrière-plan
    kill $(jobs -p) 2>/dev/null || true
    # Si un moniteur de progression a été démarré, tenter de le tuer
    if [[ -n "${monitor_pid:-}" ]]; then
        kill "${monitor_pid}" 2>/dev/null || true
    fi
    # Supprimer le FIFO de progression s'il existe (créé dans le répertoire du script)
    rm -f "${PROG_FIFO:-}"
}
...
# Normalise une valeur de bitrate : si la valeur est uniquement numérique,
# on ajoute le suffixe 'k' (ex: 2000 -> 2000k). Ne modifie pas les valeurs
# déjà munies d'un suffixe (k/K/M/m).
normalize_bitrate_var() {
    local varname="$1"
    local val="${!varname:-}"
    if [[ -z "$val" ]]; then
        return 0
    fi
    # Si la valeur est uniquement numérique, ajouter 'k'
    if [[ "$val" =~ ^[0-9]+$ ]]; then
        eval "$varname=\"${val}k\""
    fi
}

# Normalise un ensemble de variables de bitrate (pratique pour appeler en une fois)
normalize_bitrate_vars() {
    normalize_bitrate_var VIDEO_BITRATE
    normalize_bitrate_var MAXRATE
    normalize_bitrate_var BUFSIZE
}
...
_execute_nvenc_conversion() {
    ...
    # Normaliser les variables bitrate si l'utilisateur a saisi des valeurs sans suffixe
    normalize_bitrate_vars

    # Construire des options de contrôle de débit / bitrate facultatives uniquement si des variables sont définies.
    local ffmpeg_rate_opts=()
    
    # Paires option/variable : option suivie du nom de la variable qui la contrôle
    local _opts=("-b:v" "VIDEO_BITRATE" "-maxrate" "MAXRATE" "-bufsize" "BUFSIZE")
    for ((i=0; i<${#_opts[@]}; i+=2)); do
        opt=${_opts[i]}
        varname=${_opts[i+1]}
        val=${!varname:-}
        if [[ -n "$val" ]]; then
            ffmpeg_rate_opts+=( "$opt" "$val" )
        fi
    done
    unset _opts opt varname val

    if $IO_PRIORITY_CMD ffmpeg -y -loglevel warning \
        -hwaccel cuda -hwaccel_output_format cuda \
        -i "$tmp_input" \
        -c:v hevc_nvenc -preset "$NVENC_PRESET" \
        ${ffmpeg_rate_opts[@]+${ffmpeg_rate_opts[@]}} -rc vbr -cq "$CRF" \
...
export_for_parallel() {
    export -f convert_file get_video_metadata should_skip_conversion clean_number colorize_pv_output \
        ...
        normalize_bitrate_var normalize_bitrate_vars
    export DRYRUN LOG_SUCCESS LOG_SKIPPED LOG_ERROR LOG_PROGRESS SUMMARY_FILE LOG_DIR
    export TMP_DIR NVENC_PRESET CRF RC_LOOKAHEAD IO_PRIORITY_CMD SOURCE OUTPUT_DIR FFMPEG_MIN_VERSION
    export MAXRATE BUFSIZE BITRATE_CONVERSION_THRESHOLD_KBPS SKIP_TOLERANCE_PERCENT VIDEO_BITRATE
...
main() {
    ...
    # Si affichage de progression activé, créer un FIFO dans le même dossier que le script
    if [[ "$NO_PROGRESS" != true ]] && [[ "$nb_files" -gt 0 ]]; then
        PROG_FIFO="$SCRIPT_DIR/prog_fifo_${EXECUTION_TIMESTAMP}_$$.fifo"
        export PROG_FIFO
        rm -f "$PROG_FIFO" 2>/dev/null || true
        mkfifo "$PROG_FIFO"

        # Moniteur en arrière-plan : lit un message par tâche terminée et met à jour une seule ligne
        (
            local __count=0
            while IFS= read -r _line; do
                __count=$((__count + 1))
                # Effacer le reste de la ligne puis afficher la progression
                printf '\r\033[KConversion : %3d%% (%d/%d) ' $((__count * 100 / nb_files)) "$__count" "$nb_files" >&2
                if [[ $__count -ge $nb_files ]]; then
                    printf '\n' >&2
                    break
                fi
            done < "$PROG_FIFO"
    ...
    # Lancer les conversions en parallèle.
    # Rediriger la sortie des workers vers des logs pour éviter que leurs échos perturbent
    # l'affichage en une seule ligne du moniteur. Si PROG_FIFO est présent, écrire DONE dedans.
    if [[ -n "${PROG_FIFO:-}" ]]; then
        cat "$QUEUE" | xargs -0 -I{} -P "$PARALLEL_JOBS" bash -c '
            LOGFILE="$LOG_DIR/worker_${RANDOM}_$$.log";
            convert_file "$@" > "$LOGFILE" 2>&1 && printf "%s\n" DONE > "$PROG_FIFO"'
            _ {} "$OUTPUT_DIR" || true
    else
        cat "$QUEUE" | xargs -0 -I{} -P "$PARALLEL_JOBS" bash -c '
            LOGFILE="$LOG_DIR/worker_${RANDOM}_$$.log";
            convert_file "$@" > "$LOGFILE" 2>&1'
            _ {} "$OUTPUT_DIR" || true
    fi

    wait

    # Attendre et nettoyer le moniteur/FIFO si présent
    if [[ -n "${monitor_pid:-}" ]]; then
