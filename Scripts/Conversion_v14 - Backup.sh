...
build_queue() {
    ...
    # 2. Gestion du fichier queue existant
        ...
        case "$response" in
            ...
            *)
                ...
                if [[ "$LIMIT_FILES" -eq 0 && "$TEST_MODE" != true ]]; then
    ...
    local limit_count_applied=0
    local do_shuffle=false # Indique si on doit trier aléatoirement

    if [[ "$TEST_MODE" == true ]]; then
        limit_count_applied=$TEST_COUNT
        do_shuffle=true
        if [[ "$NO_PROGRESS" != true ]]; then
            echo -e "${ORANGE} LIMITATION (TEST) : Sélection aléatoire de $limit_count_applied fichiers maximum.${NOCOLOR}"
        fi
    elif [[ "$LIMIT_FILES" -gt 0 ]]; then
        limit_count_applied=$LIMIT_FILES
        if [[ "$NO_PROGRESS" != true ]]; then
            echo -e "${ORANGE} LIMITATION : Traitement de $limit_count_applied fichiers maximum.${NOCOLOR}"
        fi
    fi
    
    # Si une limite (normale ou test) a été appliquée
    if [[ "$limit_count_applied" -gt 0 ]]; then
        ...
        # 2. Appliquer le tri (aléatoire si test) et la limite
        if [[ "$do_shuffle" == true ]]; then
            # Mode TEST : Tri aléatoire
...
export_for_parallel() {
    ...
    export DRYRUN_TEST_SUFFIX SUFFIX_STRING NO_PROGRESS STOP_FLAG SCRIPT_DIR
    export TEST_MODE TEST_COUNT LIMIT_FILES CUSTOM_QUEUE EXECUTION_TIMESTAMP QUEUE
