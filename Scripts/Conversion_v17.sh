...
readonly PROGRESS_DIR="$TMP_DIR/progress"

readonly MIN_TMP_FREE_MB=2048  # Espace libre requis en MB dans /tmp

# PARAMÈTRES DE CONVERSION NVENC (HEVC/x265) définis selon le mode (voir set_conversion_mode_parameters)
# CRF (-cq) : Facteur de qualité constante. Plus haut = plus de compression / moins bonne qualité.
# RC_LOOKAHEAD : Les valeurs plus élevées consomment plus de mémoire GPU (Max. : 64)
...
readonly SURFACES=24 # 16 est une valeur sûre, plus que suffisante pour le rc-lookahead 20 et la plupart des tâches d encodage 1080p ou 4K.
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -*) 
                ...
                if [[ "$1" =~ ^-[a-zA-Z]{2,}$ ]]; then
                    # flag_to_process = le premier caractère de l option
...
Options :
  ...
  -m, --mode MODE           Mode de conversion : film, serie (ou autres) [défaut : serie]
  ...
  -r, --random              Mode aléatoire : sélectionne des fichiers aléatoires [défaut : 10]
  -l, --limit N             Limiter le traitement à N fichiers
  -q, --queue FILE          Utiliser un fichier queue personnalisé
  -n, --no-progress         Désactiver l affichage des indicateurs de progression [mode silencieux]
...
Exemples :
  ...
  ./conversion.sh -m film --random --limit 5
...
check_dependencies() {
    ...
    # Vérifier la présence de Python (python3 ou python) pour le monitor optionnel
    local py_cmd=python3
    if ! command -v "$py_cmd" &> /dev/null; then
        py_cmd=python
    fi
    if ! command -v "$py_cmd" &> /dev/null; then
        echo -e "${YELLOW}⚠️  Python non trouvé (python3/python). Le moniteur d'avancement Python sera désactivé ; fallback AWK utilisé.${NOCOLOR}"
    else
        echo -e "   - Python détecté : ${GREEN}$py_cmd${NOCOLOR}"
    fi
    ...
    echo -e "   - Mode conversion : ${CYAN}$CONVERSION_MODE${NOCOLOR} (CRF=$CRF, RC_LOOKAHEAD=$RC_LOOKAHEAD)"
...
initialize_directories() {
    mkdir -p "$LOG_DIR" "$TMP_DIR" "$OUTPUT_DIR" "$PROGRESS_DIR"
...
is_excluded() {
    ...
    # Support both array-style EXCLUDES (in-process) and exported newline-separated string
    if declare -p EXCLUDES 2>/dev/null | grep -q "declare -a"; then
        ...
        # EXCLUDES exported as a newline-separated string
        if [[ -n "${EXCLUDES:-}" ]]; then
            IFS=$'\n' read -r -d '' -a _ex_arr <<< "$(printf '%s\0' "$EXCLUDES")" 2>/dev/null || IFS=$'\n' read -r -a _ex_arr <<< "$EXCLUDES"
            for ex in "${_ex_arr[@]}"; do
                [[ -z "$ex" ]] && continue
                if [[ "$f" == "$ex"* ]]; then
                    return 0
                fi
...
colorize_pv_output() {
    local color="$1"
    sed "s/^/${color}/g;s/$/${NOCOLOR}/g" > /dev/tty
}
...
_apply_queue_limitations() {
    ...
    if [[ "$NO_PROGRESS" != true ]]; then
        if [[ "$RANDOM_MODE" == true ]]; then
            echo -e "${YELLOW} LIMITATION (RANDOM) : Sélection aléatoire de $limit_count fichiers maximum.${NOCOLOR}"
        else
            echo -e "${YELLOW} LIMITATION : Traitement de $limit_count fichiers maximum.${NOCOLOR}"
        fi
    fi
...
_display_random_mode_selection() {
    # Afficher les fichiers sélectionnés en mode aléatoire
    if [[ "$RANDOM_MODE" != true ]] || [[ "$NO_PROGRESS" == true ]]; then
        return 0
    fi
    
    echo -e "\n${CYAN}📋 Fichiers sélectionnés pour le mode aléatoire : ${NOCOLOR}"
    tr '\0' '\n' < "$QUEUE" | nl -w2 -s'. '
    echo ""
}
...
_get_temp_filename() {
    local file_original="$1"
    local suffix="$2"
    echo "$TMP_DIR/tmp_$(md5sum <<< "$file_original" | cut -c1-8)_${RANDOM}${suffix}"
}
...
_execute_nvenc_conversion() {
    local tmp_input="$1"
    local tmp_output="$2"
    local ffmpeg_log_temp="$3"
    local duration_secs="$4"
    local base_name="$5"
    # Use writer+monitor approach when PROGRESS_DIR is available and progress not disabled.
    local python_cmd=python3
    if ! command -v "$python_cmd" &> /dev/null; then python_cmd=python; fi

    # Only use the Python writer if Python is actually available; otherwise fallback to AWK
    if [[ -d "$PROGRESS_DIR" && "$NO_PROGRESS" != "true" ]] && command -v "$python_cmd" &> /dev/null; then
        local JOB_ID="${RANDOM}_$$"
        local JOB_FILE="$PROGRESS_DIR/job_${JOB_ID}.txt"
        mkdir -p "$PROGRESS_DIR" 2>/dev/null || true

        # Debug: indicate that the python writer will be used for this job
        echo "[PROGRESS] writer python enabled for ${base_name} -> ${JOB_FILE}" >&2
        ...
        "$python_cmd" -u "$SCRIPT_DIR/tools/ffmpeg_progress_writer.py" --duration "$duration_secs" --job-file "$JOB_FILE" --name "$base_name"; then
        ...
        # fallback to original awk-based progress (for compatibility)
        echo "[PROGRESS] python writer not available for ${base_name}, using AWK fallback" >&2
        if $IO_PRIORITY_CMD ffmpeg -y -loglevel warning \
            -hwaccel cuda -hwaccel_output_format cuda \
            -i "$tmp_input" \
            -c:v hevc_nvenc -preset "$NVENC_PRESET" \
...
_finalize_conversion_success() {
    ...
    local remove_original="$7"
    local sizeBeforeMB="$8"
    ...
    if [[ "$remove_original" == true ]]; then
        rm "$file_original" 2>/dev/null || true
        if [[ -n "$LOG_SUCCESS" ]]; then
            echo "$(date '+%Y-%m-%d %H:%M:%S') | SUCCESS + REMOVED | $file_original → $final_output | $size_comparison" >> "$LOG_SUCCESS" 2>/dev/null || true
        fi
    else
        if [[ -n "$LOG_SUCCESS" ]]; then
...
convert_file() {
    ...
    if _execute_nvenc_conversion "$tmp_input" "$tmp_output" "$ffmpeg_log_temp" "$duration_secs" "$base_name"; then
        _finalize_conversion_success "$filename" "$file_original" "$tmp_input" "$tmp_output" "$final_output" "$ffmpeg_log_temp" "$remove_original" "$sizeBeforeMB"
    else
...
export_for_parallel() {
    export -f convert_file get_video_metadata should_skip_conversion clean_number colorize_pv_output \
        ...
        _check_disk_space _analyze_video _copy_to_temp_storage _execute_nvenc_conversion \
    ...
    export TMP_DIR NVENC_PRESET CRF RC_LOOKAHEAD IO_PRIORITY_CMD SOURCE OUTPUT_DIR REMOVE_ORIGINAL FFMPEG_MIN_VERSION
    export MAXRATE BUFSIZE BITRATE_CONVERSION_THRESHOLD_KBPS SKIP_TOLERANCE_PERCENT 
    export MIN_TMP_FREE_MB SURFACES
    ...
    export PROGRESS_DIR
    # Export exclusions as a newline-separated string so child shells can re-split it
    export EXCLUDES="$(printf '%s\n' "${EXCLUDES[@]}")"
}
...
main() {
    ...
    # Start progress monitor (only if progress enabled)
    if [[ "$NO_PROGRESS" != true ]]; then
        PYTHON_CMD=python3
        if ! command -v "$PYTHON_CMD" &> /dev/null; then PYTHON_CMD=python; fi
        mkdir -p "$PROGRESS_DIR"
        # Démarrer le moniteur uniquement si python est disponible, sinon afficher un avertissement
        if command -v "$PYTHON_CMD" &> /dev/null; then
            # Lancer Python en mode non-buffered (-u) et forcer l'affichage sur stderr pour s'assurer
            # que les mises à jour de progression s'affichent même si stdout est redirigé.
            "$PYTHON_CMD" -u "$SCRIPT_DIR/tools/progress_monitor.py" --dir "$PROGRESS_DIR" --refresh 0.7 >&2 &
            MONITOR_PID=$!
            echo -e "${CYAN}Moniteur de progression lancé (PID ${MONITOR_PID})${NOCOLOR}" >&2
        else
            echo -e "${YELLOW}⚠️  Python introuvable : le moniteur d'avancement ne sera pas lancé (fallback AWK).${NOCOLOR}"
        fi
    fi

    # Lancer la pipeline xargs dans un sous-shell en arrière-plan pour récupérer son PID
    (
        cat "$QUEUE" | xargs -0 -I{} -P "$PARALLEL_JOBS" bash -c 'convert_file "$@"' _ {} "$OUTPUT_DIR" "$REMOVE_ORIGINAL"
    ) &
    XARGS_PID=$!

    # Attendre uniquement la fin de xargs (le moniteur tourne en tâche de fond)
    wait "$XARGS_PID" || true
    sleep 1

    # Arrêter le moniteur si lancé
