    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -h|--help)
                ...
                ;;
            -*) 
                # On vérifie si l'argument est une option courte groupée (e.g., '-xl', plus de 2 chars)
                if [[ "$1" =~ ^-[a-zA-Z]{2,}$ ]]; then
                    # flag_to_process = le premier caractère de l'option (e.g., '-x' de '-xl')
                    local flag_to_process="-${1:1:1}" 
                    # remaining_flags = le reste (e.g., '-l' de '-xl')
...
build_queue() {
    local should_generate_new_queue=true
    local current_queue_source="Générée"

    # 1. Gestion du fichier queue personnalisé (Option -q)
    if [[ -n "$CUSTOM_QUEUE" ]]; then
        ...
        should_generate_new_queue=false
        current_queue_source="Personnalisée"
    
    # 2. Gestion du fichier queue existant
    elif [[ -f "$QUEUE" ]]; then
        ...
        # On lit la réponse depuis le terminal pour éviter de consommer l'entrée de xargs/cat
        read -r -p "Souhaitez-vous conserver ce fichier queue ? (O/n) " response < /dev/tty
        ...
        case "$response" in
            [nN])
                ...
                rm -f "$QUEUE"
                should_generate_new_queue=true
                ;;
            *)
                if [[ "$NO_PROGRESS" != true ]]; then
                    echo -e "${YELLOW}Utilisation de la file d'attente existante.${NOCOLOR}"
                fi
                
                # S'il n'y a PAS de limite à appliquer, on peut sortir immédiatement.
                if [[ "$LIMIT_FILES" -eq 0 && "$RANDOM_MODE" != true ]]; then
                    if ! [[ -s "$QUEUE" ]]; then echo "Aucun fichier à traiter trouvé."; exit 0; fi
                    return 0
                fi
                # Sinon, on continue pour appliquer la limite
                should_generate_new_queue=false
                current_queue_source="Conservée"
                ;;
        esac
    fi
    
    # 3. Génération de la queue (si nécessaire)
    if [[ "$should_generate_new_queue" == true ]]; then
        local exclude_dir_name=$(basename "$OUTPUT_DIR")
    
        # Première passe : compter le nombre total de fichiers vidéo candidats (pour l'affichage)
        if [[ "$NO_PROGRESS" != true ]]; then echo "Indexation fichiers..." >&2; fi
        ...
        while IFS= read -r -d $'\0' f; do
            ...
            # Stockage de la taille et du chemin
            echo -e "$(stat -c%s "$f")\t$f"
        done > "$QUEUE.tmp"
        ...
        # Tri initial par taille décroissante
        sort -nrk1,1 "$QUEUE.tmp" | cut -f2- | tr '\n' '\0' > "$QUEUE"
        rm "$QUEUE.tmp"
    fi
    
    # 4. APPLICATION DE LA LIMITATION (Unifiée, s'applique à la queue prete, peu importe sa source)
    local limit_count_applied=$LIMIT_FILES
    local do_shuffle=$RANDOM_MODE # En mode random, trier aléatoirement

    if [[ "$limit_count_applied" -gt 0 ]]; then
        if [[ "$RANDOM_MODE" == true ]]; then
            if [[ "$NO_PROGRESS" != true ]]; then
                echo -e "${YELLOW} LIMITATION (RANDOM) : Sélection aléatoire de $limit_count_applied fichiers maximum.${NOCOLOR}"
            fi
        else
            if [[ "$NO_PROGRESS" != true ]]; then
                echo -e "${YELLOW} LIMITATION : Traitement de $limit_count_applied fichiers maximum.${NOCOLOR}"
            fi
        fi
    fi
    
    # Si une limite (normale ou random) a été appliquée
    if [[ "$limit_count_applied" -gt 0 ]]; then
        local tmp_limit="$QUEUE.limit"
        
        # 1. Lire la queue (séparée par \0) et la convertir en lignes pour le traitement
        local queue_content
        queue_content=$(tr '\0' '\n' < "$QUEUE")
        
        # 2. Appliquer le tri (aléatoire si random) et la limite
        if [[ "$do_shuffle" == true ]]; then
            # Mode RANDOM : Tri aléatoire
            echo "$queue_content" | sort -R | head -n "$limit_count_applied" | tr '\n' '\0' > "$tmp_limit"
        else
            # Mode Normal : Juste la tête de la liste (déjà triée par taille)
            echo "$queue_content" | head -n "$limit_count_applied" | tr '\n' '\0' > "$tmp_limit"
        fi
    ...
    # En mode random, afficher les fichiers sélectionnés aléatoirement
    if [[ "$RANDOM_MODE" == true ]]; then
        if [[ "$NO_PROGRESS" != true ]]; then
    ...
    # Créer une version lisible pour consultation (inclut la limitation)
...
show_summary() {
    local succ=$(grep -c ' | SUCCESS' "$LOG_SUCCESS" 2>/dev/null || echo "0")
    local skip=$(grep -c ' | SKIPPED' "$LOG_SKIPPED" 2>/dev/null || echo "0")
...
export_for_parallel() {
    ...
    export RANDOM_MODE RANDOM_MODE_DEFAULT_LIMIT LIMIT_FILES CUSTOM_QUEUE EXECUTION_TIMESTAMP QUEUE
...
main() {
    ...
    cat "$QUEUE" | xargs -0 -I{} -P "$PARALLEL_JOBS" bash -c 'convert_file "$@"' _ {} "$OUTPUT_DIR" "$REMOVE_ORIGINAL" || true
