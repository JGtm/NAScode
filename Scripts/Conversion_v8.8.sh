...
# Suffixe additionnel pour les fichiers créés en mode Dry Run
DRYRUN_TEST_SUFFIX="-dryrun-sample"
...
QUEUE="$LOG_DIR/Queue"
LOG_DRYRUN_COMPARISON="$LOG_DIR/DryRun_Comparison_${EXECUTION_TIMESTAMP}.log" # NOUVEAU LOG
...
for cmd in ffmpeg ffprobe pv; do
    if ! command -v $cmd &> /dev/null; then echo -e "${RED}ERREUR: $cmd introuvable.${NOCOLOR}"; exit 1; fi
done

if [ ! -d "$SOURCE" ]; then echo -e "${RED}ERREUR: Source '$SOURCE' introuvable.${NOCOLOR}"; exit 1; fi

mkdir -p "$LOG_DIR" "$TMP_DIR" "$OUTPUT_DIR"
touch "$LOG_SUCCESS" "$LOG_SKIPPED" "$LOG_ERROR" "$SUMMARY_FILE" "$LOG_PROGRESS" "$QUEUE" "$LOG_DRYRUN_COMPARISON"
...
convert_file() {
    ...
    # LOGIQUE SUFFIXE
    # POSSIBLEMENT DOUBLON AVEC LIGNE 558
	# V2 Détermination du suffixe EFFECTIF
    local effective_suffix="$SUFFIX_STRING"
    if [[ "$DRYRUN" == true ]]; then
        effective_suffix="${effective_suffix}${DRYRUN_TEST_SUFFIX}"
    fi

    # Utilisation du suffixe EFFECTIF pour le nom de sortie
...
dry_run_compare_names() {
    ...
    local TTY_DEV="/dev/tty" # Terminal du script (pour l'affichage en mode log)
    local LOG_FILE="$LOG_DRYRUN_COMPARISON" # Fichier log de la comparaison
    ...
    case "$response" in
        [oO]|[yY]|'')
            # Le header est envoyé vers le terminal ET le log
            {
                echo ""
                echo "-------------------------------------------"
            ...
            while IFS= read -r -d $'\0' file_original; do
                ...
                # Le reste du calcul du chemin et du nom final est inchangé
                local relative_path="${file_original#$SOURCE}"
                relative_path="${relative_path#/}"
                local relative_dir=$(dirname "$relative_path")
                local final_dir="$OUTPUT_DIR/$relative_dir"
                ...
                # DÉMARRAGE DU BLOC LOGUÉ : tout ce qui est dans les accolades sera envoyé à la fois au log et au terminal
                {
                    ...
                    if [[ -n "$anomaly_message" ]]; then
                        echo "$anomaly_message" # Affiché en clair dans le log
                        echo -e "${RED}  $anomaly_message${NOCOLOR}" > $TTY_DEV # Affiché en couleur dans le terminal
                    fi
                    
                    # Affichage des noms
                    printf "  %-10s : %s\n" "ORIGINAL" "$filename" # Affiché en clair dans le log
                    printf "  %-10s : %s\n" "GÉNÉRÉ" "$final_output_basename" # Affiché en clair dans le log
