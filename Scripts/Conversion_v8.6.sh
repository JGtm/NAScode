...
check_output_suffix() {
    ...
    case "$response" in
        [nN])
            # L'utilisateur choisit de ne PAS utiliser de suffixe
            SUFFIX_STRING=""
            echo -e "${YELLOW}⚠️  Le suffixe de sortie est désactivé.${NOCOLOR}"
            ;;
        *)
            # L'utilisateur choisit d'utiliser le suffixe par défaut (ou un autre s'il a été changé manuellement)
...
convert_file() {
    ...
    if [[ "$DRYRUN" != true ]] && [[ -f "$final_output" ]]; then
        echo -e "   ${CYAN}⏭️ SKIPPED (Fichier de sortie existe déjà) : $filename${NOCOLOR}"
    ...
    if [[ -z "$codec" ]]; then
        echo -e "   ${CYAN}⏭️ SKIPPED (Pas de flux vidéo) : $filename${NOCOLOR}"
    ...
    if [[ "$codec" == "hevc" || "$codec" == "h265" ]]; then
        if [[ "$bitrate" =~ ^[0-9]+$ ]] && [[ "$bitrate" -le "$max_tolerated_bits" ]]; then
            echo -e "   ${CYAN}⏭️ SKIPPED (Déjà x265 & bitrate optimisé) : $filename${NOCOLOR}"
...
export DRYRUN LOG_SUCCESS LOG_SKIPPED LOG_ERROR LOG_PROGRESS SUMMARY_FILE TMP_DIR NVENC_PRESET CRF IO_PRIORITY_CMD SOURCE OUTPUT_DIR REMOVE_ORIGINAL MAXRATE BUFSIZE BITRATE_CONVERSION_THRESHOLD_KBPS SKIP_TOLERANCE_PERCENT MIN_TMP_FREE_MB RC_LOOKAHEAD SURFACES NOCOLOR GREEN YELLOW RED CYAN MAGENTA
