...
convert_file() {
    ...
    if [[ -z "$codec" ]]; then
		...
		echo "   ⏭️ SKIPPED (Pas de flux vidéo)"
		echo ""
        return 0
    fi
    ...
    if [[ "$codec" == "hevc" || "$codec" == "h265" ]]; then
        if [[ "$bitrate" =~ ^[0-9]+$ ]] && [[ "$bitrate" -le "$max_tolerated_bits" ]]; then
			...
			echo "   ⏭️ SKIPPED (Déjà x265 & bitrate optimisé)"
			echo ""
            return 0
        fi
    ...
    # ----------------------------------------------------
    # CONVERSION GPU NVENC AVEC AFFICHAGE DYNAMIQUE
    # ----------------------------------------------------
    echo "  → Encodage NVENC"
    ...
    if $IO_PRIORITY_CMD ffmpeg -y -loglevel warning \
        ...
        -map 0 -f matroska \
        "$tmp_output" \
        -progress pipe:1 2> "$ffmpeg_log_temp" | \
