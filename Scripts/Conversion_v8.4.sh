...
# ====> Corrigé via l'ajout de -pix_fmt yuv420p
###########################################################
...
# ----------------------------------------------------
# INDIQUER A PLEX D'IGNORER LE DOSSIER CONVERTED
# ----------------------------------------------------

check_plexignore() {
    # Lit les chemins absolus pour une comparaison fiable, même avec '.'
    local source_abs
    ...
    # Vérifie si le dossier de sortie est à l'intérieur du dossier source
    # La condition 'if [[ "$output_abs"/ != "$source_abs"/ ]]' s'assure qu'ils ne sont pas les mêmes
    # La condition '[[ "$output_abs"/ = "$source_abs"/* ]]' vérifie que $OUTPUT_DIR est un sous-dossier de $SOURCE
    if [[ "$output_abs"/ != "$source_abs"/ ]] && [[ "$output_abs" = "$source_abs"/* ]]; then
        
        # Vérifie si l'utilisateur n'a pas sélectionné de supprimer les originaux
        if [[ "$REMOVE_ORIGINAL" == false ]]; then
            
            if [[ -f "$plexignore_file" ]]; then
                echo -e "\nℹ️  Fichier .plexignore déjà présent dans '$OUTPUT_DIR'. Aucune action requise."
                return 0
            fi

            echo ""
            read -r -p "Souhaitez-vous créer un fichier .plexignore dans '$OUTPUT_DIR' pour éviter les doublons sur Plex ? (Y/N) " response
            
            case "$response" in
                [yY]|[nN]|'')
                    echo "*" > "$plexignore_file"
                    echo -e "✅ Fichier .plexignore créé dans '$OUTPUT_DIR' pour masquer les doublons."
                    ;;
                *)
                    echo -e "⏭️  Création de .plexignore ignorée."
                    ;;
            esac
        fi
    fi
...
convert_file() {
    ...
    if [[ -z "$codec" ]]; then
        echo "   ⏭️ SKIPPED (Pas de flux vidéo) : $filename"
    ...
    if [[ "$codec" == "hevc" || "$codec" == "h265" ]]; then
        if [[ "$bitrate" =~ ^[0-9]+$ ]] && [[ "$bitrate" -le "$max_tolerated_bits" ]]; then
            echo "   ⏭️ SKIPPED (Déjà x265 & bitrate optimisé) : $filename"
    ...
    echo "  → Transfert de [$filename] vers dossier temporaire..."
    ...
    awk -v DURATION="$duration_secs" -v CURRENT_FILE_NAME="$filename" '
        BEGIN {
            duration = DURATION;
            last_printed = -10;
            if (duration < 1) { exit; }
        ...
        /out_time_us=/ {
            ...
            if (p - last_printed >= 5 || (p >= 99 && last_printed < 95)) {
                printf "    ... [%s] Progression : %.0f%%\n", CURRENT_FILE_NAME, p;
                fflush();
                last_printed = p;
            }
        }
        /progress=end/ {
            printf "    ... [%s] Progression : 100%%\n", CURRENT_FILE_NAME;
            fflush();
        }
    ' ; then 
        ...
        echo ""; echo "  ❌ Échec de la conversion : $filename"
...
err=$(grep -c ' | ERROR ffmpeg | ' "$LOG_ERROR")
{
  echo ""
  echo "-------------------------------------------"
