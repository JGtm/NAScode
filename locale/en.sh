...
MSG_CFG_AVAILABLE_MODES="Available modes: film, adaptatif, adaptatif-vmaf, gaming, serie"
MSG_CFG_TWOPASS_IGNORED_ADAPTIVE="Two-pass (-2) ignored in %s mode: this mode encodes with single-pass constrained CRF (bitrate computed per file/segment)."
...
MSG_HELP_ESSENTIAL="force the SVT-AV1-Essential binary (AV1 encoding)"
MSG_HELP_NO_ESSENTIAL="force the mainline libsvtav1 fallback"
...
MSG_HELP_KEEP_METADATA="Preserve source file metadata and modification date (FLAG)"
...
MSG_HELP_MODE_ADAPTATIF_VMAF="per-segment auto-boost-lite via predictive VMAF (AV1, audio passthrough)"
MSG_HELP_MODE_GAMING="adaptatif variant tuned for high-motion (replays, game captures; any FPS)"
...
MSG_UI_OPT_KEEP_METADATA="Preserving source metadata and modification date"
...
MSG_UI_FPS_LIMITED="FPS limited: %s → %s fps"
